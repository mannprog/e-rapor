@extends('admin.layouts.app', ['title' => 'Kelola Kelas'])
...
@section('content')
    ...
    @include('admin.pages.sistem.kelas.component.addOrEdit')
@endsection
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click', '#editData', function() {
                ...
                $.get("{{ route('sistem.kelas.index') }}" + '/' + item_id + '/edit', function(data) {
                    $('#modal-ed').modal('show');
                    setTimeout(function() {
                        $('#nama').focus();
                    }, 500);
            ...
            $('#editBtn').click(function(e) {
                ...
                $.ajax({
                    data: formData,
                    url: "{{ route('sistem.kelas.index') }}" + '/' + item_id,
                    contentType: false,
                    processData: false,
                    type: "POST",
                    success: function(data) {
            ...
            $('body').on('click', '.deleteBtn', function(e) {
                ...
                if (confirmation) {
                    ...
                    $.ajax({
                        data: formData,
                        url: "{{ route('sistem.kelas.index') }}" + '/' + kelas_id,
                        contentType: false,
                        processData: false,
                        type: "POST",
                        success: function(data) {
